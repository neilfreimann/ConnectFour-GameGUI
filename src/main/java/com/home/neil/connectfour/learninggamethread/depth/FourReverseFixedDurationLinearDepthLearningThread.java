...
public class FourReverseFixedDurationLinearDepthLearningThread extends StatefulFixedDurationLearningThread  {
	public static final String CLASS_NAME = FourReverseFixedDurationLinearDepthLearningThread.class.getName();
	public static final String PACKAGE_NAME = CLASS_NAME.substring(0, CLASS_NAME.lastIndexOf("."));
	public static final String MBEAN_NAME = PACKAGE_NAME + ":type=" + FourReverseFixedDurationLinearDepthLearningThread.class.getSimpleName();

	public static Logger sLogger = LogManager.getLogger(PACKAGE_NAME);

	private final static String FILENAME_FIXED_DURATION_LINEAR_DEPTH = FourReverseFixedDurationLinearDepthLearningThread.class.getSimpleName() + ".dat";
	
	private static FourReverseFixedDurationLinearDepthLearningThread sInstance = null;

	public synchronized void renameThread(String pLogContext) {
		sLogger.trace("Entering");
		sThreadNumber++;

		setName(FourReverseFixedDurationLinearDepthLearningThread.class.getSimpleName() + "." + sThreadNumber);

		if (pLogContext == null || pLogContext.trim().isEmpty()) {
			mLogContext = FourReverseFixedDurationLinearDepthLearningThread.class.getSimpleName() + "." + sThreadNumber;
	...
	public static synchronized FourReverseFixedDurationLinearDepthLearningThread getInstance(KnowledgeBaseFilePool pKnowledgeBaseFilePool, long pDurationToRunInMs, String pLogContext) throws ConfigurationException, IOException {
		sLogger.trace("Entering");

		if (sInstance == null) {
			sInstance = new FourReverseFixedDurationLinearDepthLearningThread(pKnowledgeBaseFilePool, pDurationToRunInMs, pLogContext);
	...
	private FourReverseFixedDurationLinearDepthLearningThread(KnowledgeBaseFilePool pKnowledgeBaseFilePool, long pDurationToRunInMs, String pLogContext) throws ConfigurationException, FileNotFoundException, IOException{
