...
public class FixedDurationLinearWidthLearningThread extends StatefulFixedDurationLearningThread {
	public static final String CLASS_NAME = FixedDurationLinearWidthLearningThread.class.getName();
	public static final String PACKAGE_NAME = CLASS_NAME.substring(0, CLASS_NAME.lastIndexOf("."));
	public static final String MBEAN_NAME = PACKAGE_NAME + ":type=" + FixedDurationLinearWidthLearningThread.class.getSimpleName();
	
	public static Logger sLogger = LogManager.getLogger(PACKAGE_NAME);

	private final static String FILENAME_FIXED_DURATION_LINEAR_WIDTH = FixedDurationLinearWidthLearningThread.class.getSimpleName() + ".dat";
	...
	private static FixedDurationLinearWidthLearningThread sInstance = null;
	
	public synchronized void renameThread(String pLogContext) {
		sLogger.trace("Entering");
		sThreadNumber++;

		setName(FixedDurationLinearWidthLearningThread.class.getSimpleName() + "." + sThreadNumber);

		if (pLogContext == null || pLogContext.trim().isEmpty()) {
			mLogContext = FixedDurationLinearWidthLearningThread.class.getSimpleName() + "." + sThreadNumber;
	...
	public static synchronized FixedDurationLinearWidthLearningThread getInstance(KnowledgeBaseFilePool pKnowledgeBaseFilePool, long pDurationToRunInMs, String pLogContext) throws ConfigurationException, IOException {
		sLogger.trace("Entering");

		if (sInstance == null) {
			sInstance = new FixedDurationLinearWidthLearningThread(pKnowledgeBaseFilePool, pDurationToRunInMs, pLogContext);
	...
	private FixedDurationLinearWidthLearningThread(KnowledgeBaseFilePool pKnowledgeBaseFilePool, long pDurationToRunInMs, String pLogContext) throws ConfigurationException, FileNotFoundException, IOException {
