...
public class FiveReverseFixedDurationLinearDepthLearningThread extends StatefulFixedDurationLearningThread  {
	public static final String CLASS_NAME = FiveReverseFixedDurationLinearDepthLearningThread.class.getName();
	public static final String PACKAGE_NAME = CLASS_NAME.substring(0, CLASS_NAME.lastIndexOf("."));
	public static final String MBEAN_NAME = PACKAGE_NAME + ":type=" + FiveReverseFixedDurationLinearDepthLearningThread.class.getSimpleName();

	public static Logger sLogger = LogManager.getLogger(PACKAGE_NAME);

	private final static String FILENAME_FIXED_DURATION_LINEAR_DEPTH = FiveReverseFixedDurationLinearDepthLearningThread.class.getSimpleName() + ".dat";
	
	private static FiveReverseFixedDurationLinearDepthLearningThread sInstance = null;

	public synchronized void renameThread(String pLogContext) {
		sLogger.trace("Entering");
		sThreadNumber++;

		setName(FiveReverseFixedDurationLinearDepthLearningThread.class.getSimpleName() + "." + sThreadNumber);

		if (pLogContext == null || pLogContext.trim().isEmpty()) {
			mLogContext = FiveReverseFixedDurationLinearDepthLearningThread.class.getSimpleName() + "." + sThreadNumber;
	...
	public static synchronized FiveReverseFixedDurationLinearDepthLearningThread getInstance(KnowledgeBaseFilePool pKnowledgeBaseFilePool, long pDurationToRunInMs, String pLogContext) throws ConfigurationException, FileNotFoundException, IOException {
		sLogger.trace("Entering");

		if (sInstance == null) {
			sInstance = new FiveReverseFixedDurationLinearDepthLearningThread(pKnowledgeBaseFilePool, pDurationToRunInMs, pLogContext);
	...
	private FiveReverseFixedDurationLinearDepthLearningThread(KnowledgeBaseFilePool pKnowledgeBaseFilePool, long pDurationToRunInMs, String pLogContext) throws ConfigurationException,FileNotFoundException, IOException{
