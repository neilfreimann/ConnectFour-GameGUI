...
public class FixedDurationBestThreeMoveBlockLearningThread extends StatefulFixedDurationLearningThread {
	...
	private void performExhaustiveSearch(BoardState pBoardStateToExpand, String pStack, String pBreadCrumbs) throws KnowledgeBaseException {
		sLogger.trace("Entering");
		...
		if (lSubBoardStates != null) {
			...
			ArrayList<BoardState> lSubBoardStatesSublist = new ArrayList<BoardState>();
			
			int count = 0;
			for (Iterator<BoardState> lIterator = lSubBoardStates.iterator(); lIterator.hasNext();) {
				BoardState lBoardState = lIterator.next();
			...
			for (Iterator<BoardState> lIterator = lSubBoardStatesSublist.iterator(); lIterator.hasNext();) {
				...
				if (lSubBoardStatesLevel2Sub != null) {
					...
					ArrayList<BoardState> lSubBoardStatesLevel2SubList = new ArrayList<BoardState>();
					
					count = 0;
					for (Iterator<BoardState> lIterator2 = lSubBoardStatesLevel2Sub.iterator(); lIterator2.hasNext();) {
						BoardState lBoardState = lIterator2.next();
