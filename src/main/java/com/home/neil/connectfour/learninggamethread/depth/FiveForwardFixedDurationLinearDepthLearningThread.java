...
public class FiveForwardFixedDurationLinearDepthLearningThread extends StatefulFixedDurationLearningThread {
	public static final String CLASS_NAME = FiveForwardFixedDurationLinearDepthLearningThread.class.getName();
	public static final String PACKAGE_NAME = CLASS_NAME.substring(0, CLASS_NAME.lastIndexOf("."));
	public static final String MBEAN_NAME = PACKAGE_NAME + ":type=" + FiveForwardFixedDurationLinearDepthLearningThread.class.getSimpleName();

	public static Logger sLogger = LogManager.getLogger(PACKAGE_NAME);

	private final static String FILENAME_FIXED_DURATION_LINEAR_DEPTH = FiveForwardFixedDurationLinearDepthLearningThread.class.getSimpleName() + ".dat";

	private static FiveForwardFixedDurationLinearDepthLearningThread sInstance = null;

	public synchronized void renameThread(String pLogContext) {
		sLogger.trace("Entering");
		sThreadNumber++;

		setName(FiveForwardFixedDurationLinearDepthLearningThread.class.getSimpleName() + "." + sThreadNumber);

		if (pLogContext == null || pLogContext.trim().isEmpty()) {
			mLogContext = FiveForwardFixedDurationLinearDepthLearningThread.class.getSimpleName() + "." + sThreadNumber;
	...
	public static synchronized FiveForwardFixedDurationLinearDepthLearningThread getInstance(KnowledgeBaseFilePool pKnowledgeBaseFilePool,long pDurationToRunInMs, String pLogContext) throws ConfigurationException, IOException {
		sLogger.trace("Entering");

		if (sInstance == null) {
			sInstance = new FiveForwardFixedDurationLinearDepthLearningThread(pKnowledgeBaseFilePool,pDurationToRunInMs, pLogContext);
	...
	private FiveForwardFixedDurationLinearDepthLearningThread(KnowledgeBaseFilePool pKnowledgeBaseFilePool, long pDurationToRunInMs, String pLogContext) throws ConfigurationException, FileNotFoundException, IOException{
	...
	public void runLearningThread() {
		...
		for (int i = 0; i < mBreadCrumbs.length(); i++) {
			...
			if (mCurrentBoardState == null) {
				...
				ExpansionTask lExpansionTask = new ExpansionTask(mCurrentBoardState, mLogContext);
		...
		while (!isTimeToTerminate()) {
			...
			if (lSubBoardStates == null || lSubBoardStates.isEmpty()) {
				...
				while (!lFoundNextToEvaluate && mCurrentBoardState != null) {
					...
					ExpansionTask lExpansionTask2 = new ExpansionTask(mCurrentBoardState, mLogContext);
					boolean lSuccess2 = lExpansionTask2.executeTask();
					...
					lSubBoardStates = lExpansionTask2.getSubBoardStates();
