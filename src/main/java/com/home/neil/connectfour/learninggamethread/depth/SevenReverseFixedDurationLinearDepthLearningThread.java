...
public class SevenReverseFixedDurationLinearDepthLearningThread extends StatefulFixedDurationLearningThread {
	public static final String CLASS_NAME = SevenReverseFixedDurationLinearDepthLearningThread.class.getName();
	public static final String PACKAGE_NAME = CLASS_NAME.substring(0, CLASS_NAME.lastIndexOf("."));
	public static final String MBEAN_NAME = PACKAGE_NAME + ":type=" + SevenReverseFixedDurationLinearDepthLearningThread.class.getSimpleName();

	public static Logger sLogger = LogManager.getLogger(PACKAGE_NAME);

	private final static String FILENAME_FIXED_DURATION_LINEAR_DEPTH = SevenReverseFixedDurationLinearDepthLearningThread.class.getSimpleName() + ".dat";

	private static SevenReverseFixedDurationLinearDepthLearningThread sInstance = null;

	public synchronized void renameThread(String pLogContext) {
		sLogger.trace("Entering");
		sThreadNumber++;

		setName(SevenReverseFixedDurationLinearDepthLearningThread.class.getSimpleName() + "." + sThreadNumber);

		if (pLogContext == null || pLogContext.trim().isEmpty()) {
			mLogContext = SevenReverseFixedDurationLinearDepthLearningThread.class.getSimpleName() + "." + sThreadNumber;
	...
	public static synchronized SevenReverseFixedDurationLinearDepthLearningThread getInstance(KnowledgeBaseFilePool pKnowledgeBaseFilePool, long pDurationToRunInMs, String pLogContext) throws ConfigurationException, IOException {
		sLogger.trace("Entering");

		if (sInstance == null) {
			sInstance = new SevenReverseFixedDurationLinearDepthLearningThread(pKnowledgeBaseFilePool, pDurationToRunInMs, pLogContext);
	...
	private SevenReverseFixedDurationLinearDepthLearningThread(KnowledgeBaseFilePool pKnowledgeBaseFilePool, long pDurationToRunInMs, String pLogContext) throws ConfigurationException, FileNotFoundException, IOException{
